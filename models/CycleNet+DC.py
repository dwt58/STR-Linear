...
class STR(nn.Module):
    ...
    def __init__(self, num_nodes, seq_len, refiner_dim=256):
        ...
        # 注意：SE 注意力块 feature_recalibrate 已被完全移除
    ...
    def forward(self, x):
        ...
        # 注意：此处跳过 SE 注意力重标定步骤，直接使用 fused 特征

        # --- 输出投影 ---
        output = self.output_conv(fused)  # (B, seq_len, N)
